...
export class EditorManager {
    ...
    setupEditor(initialLanguage) {  // 파라미터 추가
        this.currentLanguage = initialLanguage;  // 초기 언어 설정
        const defaultLang = SUPPORTED_LANGUAGES[this.currentLanguage];

        this.editor = monaco.editor.create(document.getElementById(this.containerId), {
            value: defaultLang.defaultContent,
            language: this.currentLanguage,  // 선택된 언어 적용
            theme: 'vs-dark',
    ...
    setLanguage(languageId) {
        ...
        if (!currentContent) {
            this.setValue(SUPPORTED_LANGUAGES[languageId].defaultContent);
        }
